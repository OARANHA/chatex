...
export class InstagramChannel extends BaseChannel {
  ...
  async sendMessage(from: string, to: string, content: MessageContent): Promise<ApiResponse<Message>> {
    ...
    try {
      // Implementação usando Instagram Graph API
      const payload = this.buildInstagramPayload(to, content);
      
      const response = await this.retryOperation(async () => {
        return this.httpClient.post(`/instagram/messages`, payload);
      });
  ...
  private buildInstagramPayload(to: string, content: MessageContent): any {
    const basePayload = {
      recipient: { id: to },
    ...
    switch (content.type) {
      ...
      default:
        throw new Error(`Instagram Channel: Content type '${content.type}' not supported`);
    }
  }
}